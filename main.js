    ...
    if (msg.fromMe) {
        if (msg.body == "!allow" && config.pmpermit_enabled == "true" && !msg.to.includes("-")) { // allow and unmute the chat (PMPermit module)
            ...
            exec("cd public && " + msg.body.replace("!term ", ""), (error, stdout, stderr) => {
                if (error) {
                    client.sendMessage(msg.to, "*ck-whatsbot~:* ```" + error + "```")
                } else if (stderr) {
                    client.sendMessage(msg.to, "*ck-whatsbot~:* ```" + stderr + "```")
                } else {
                    client.sendMessage(msg.to, "*ck-whatsbot~:* ```" + stdout + "```")
                }
            })
            ...
            client.sendMessage(msg.to, new MessageMedia(data.mimetype, data.data, data.filename), { caption: `QR code for 馃憞\n` + "```" + msg.body.replace("!qr ", "") + "```" });
            ...
            if (data == "error") {
                client.sendMessage(msg.to, `馃檱鈥嶁檪锔� *Error*\n\n` + "```Something Unexpected Happened to fetch this Zee5 Content, Maybe it's a wrong url.```")
            } else {
                client.sendMessage(msg.to, new MessageMedia(data.image.mimetype, data.image.data, data.image.filename), { caption: `馃帴 *${data.title}* _(${data.genre})_\n\n馃搫 ` + "```" + data.description + "```" + `\n\n*Stream Url* 馃憞\n${data.url}` });
            }
            ...
            if (data == "error") {
                ...
                client.sendMessage(msg.to, new MessageMedia(data.image.mimetype, data.image.data, data.image.filename), { caption: `馃幎 *${data.title}* _(${data.released_year})_\n\n馃搥 *Artist :*  ` + "```" + data.singers + "```\n馃摎 *Album :*  " + "```" + data.album + "```" + `\n\n*Download Url* 馃憞\n${data.url}` });
            }
            ...
            if (data == "error") {
                ...
                client.sendMessage(msg.to, new MessageMedia(data.mimetype, data.data, data.filename), { caption: `Carbon for 馃憞\n` + "```" + msg.body.replace("!carbon ", "") + "```" });
            }
            ...
            if (data == "error") {
                ...
                quotedMsg.reply(`馃敆 *Direct Link 馃憞*\n\n` + "```" + data + "```")
            }
            ...
            if (data == "error") {
                ...
                client.sendMessage(msg.to, `*Today's Weather at ${data.place}*\n` + "```" + data.current_observation.text + " (" + data.current_observation.temperature + "掳C)```\n\n*Type:* " + "```" + data.today_forcast.text + "```\n*Max temperature:* " + "```" + data.today_forcast.high + "掳C```\n*Min temperature:* " + "```" + data.today_forcast.low + "掳C```");
            }
...
client.on('message_revoke_everyone', async(after, before) => {
    if (before) {
        if (before.fromMe !== true && before.hasMedia !== true && before.author == undefined && config.enable_delete_alert == "true") {
            client.sendMessage(before.from, "_You deleted this message_ 馃憞馃憞\n\n" + before.body)
        }
    }
});
...
app.get('/', (req, res) => {
    res.send('<h1>This server is powered by cyberkallan<br><a href="https://github.com/cyberkallanyt/ckbot">https://github.com/cyberkallanyt/ckbot</a></h1>')
})
